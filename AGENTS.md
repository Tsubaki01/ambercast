...
- Directory reliability: where session `cwd` reliability is in doubt, prefer absolute paths and `git -C <absolute-worktree-path> <verb>` as a single command for git's own targeting correctness; this does not change which directory `guard_git.py` evaluates. If a commit/push is unexpectedly blocked while the intended worktree is on the expected branch, verify it directly (`git -C <absolute-worktree-path> rev-parse --abbrev-ref HEAD`) and report to the orchestrator rather than switching between `-C` and a leading `cd`; neither form repairs a mismatch in the guard's resolved directory.
