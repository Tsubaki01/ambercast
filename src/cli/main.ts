 ...
 * rows always have the same status, so their healthy styling must follow the
 * settled application state: applied, preview-only, and no-artifact-change are
 * healthy; every other application is not. The renderer appends a result's optional `reason`; check supplies only its fixed,
