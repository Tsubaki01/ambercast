...
- Teammates run Codex synchronously (no `--background`): an in-process teammate cannot own background work ("a teammate's background work can't outlive the lead's process") and may never receive a completion wake-up (upstream claude-code#23415). Background delegation belongs to the lead session only.
- A watchdog (`.claude/hooks/watch_progress.py`, armed automatically at every turn end as an asyncRewake Stop hook) force-wakes the lead — even from idle — when the flow makes no progress for 12 minutes. On a watchdog wake: check Codex jobs and teammate status, re-engage idle teammates by name via SendMessage (idle teammates are hidden, not stopped), then continue the flow. `AMBERCAST_WATCHDOG=0` disables it.
