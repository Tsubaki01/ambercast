...
2. **Branch** — Create `issues/<N>` from up-to-date `main` (`git switch -c issues/<N>`). Hooks reject commits on `main` and on non-`issues/<N>` branches. → `step02_branch=done`
3. **Plan** — Write the implementation plan to `.claude/impl/issue-<N>-plan.md`: scope, affected files, component/module design, algorithms, data structures, test strategy, out-of-scope notes. → `step03_plan=done`
...
13. **Push** — Push the branch. → `step13_push=done`
14. **CI** — Watch CI (`gh run watch` / `gh pr checks`); fix until everything is green. → `step14_ci=done`
15. **PR** — Open a PR referencing the issue (`Closes #<N>`), summarizing plan, reviews, and test coverage. → `step15_pr=done`
16. **CodeRabbit** — Wait for the CodeRabbit review (poll `gh pr view --comments`), address every finding, resolve all conversations. → `step16_coderabbit=done`
17. **Merge** — Merge via GitHub (branch protection requires the PR checks and resolved conversations). Delete the branch. → `step17_merged=done`
